...
def main():      
    ...
    for step_idx, exp in enumerate(exp_collector.rollout()):
        ...
        b_states = torch.stack(exp['states'], dim=0).view(-1, envs.single_observation_space.shape[0])
        b_actions = torch.stack(exp['actions'], dim=0).view(-1, envs.single_action_space.shape[0])
        b_log_probs = torch.stack(exp['log_probs'], dim=0).view(-1)
        b_returns = exp['returns'].view(-1)
        b_values = torch.stack(exp['values'],dim=0).view(-1)
        b_advs = exp['adv'].view(-1)
        ...
        for i in range(PPO_EPOCHS):
            ...
            for start in range(0, batch_size, MINI_BATCH_SIZE):
                ...
                log_prob_new = dist_new.log_prob(mb_actions).sum(dim=-1)
                ...
                loss_policy = torch.max(pg_loss1, pg_loss2).mean()
                ...
                # loss_value = F.smooth_l1_loss(value,mb_returns.detach(), beta=1.0)
        ...
        wandb.log({
                ...
                # --- Action Diagnostics ---
                "action_saturation": (b_actions.abs() > 0.99).float().mean().item(), # CRITICAL METRIC
                "action_mean": b_actions.mean().item(),
                "action_std": b_actions.std().item(),
